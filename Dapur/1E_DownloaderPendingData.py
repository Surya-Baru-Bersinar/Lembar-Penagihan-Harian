...
def main():
    ...
    section = 'PENDING'
    output_name = 'Pending_temp.xlsx'

    if piutang.has_section(section):
        pend_stats = piutang.get(section, 'pend_stats', fallback='')
        
        if str(pend_stats).strip().lower() == 'ya':
            url = piutang.get(section, 'pend_url', fallback='')
            download_sheet(url, output_name)
        else:
            print(f"--> Status pend_stats pada [{section}] bernilai '{pend_stats}'. Proses untuk {output_name} dilewati.")
    else:
